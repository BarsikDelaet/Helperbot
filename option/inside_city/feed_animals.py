...
from keyboards.city.for_feed_animals import get_keyboard_feed_animals
...
class FeedAnimals(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_feed_animals(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_CITY,
                              reply_markup=get_keyboard_feed_animals())
        self.bot.register_next_step_handler(msg, self.choice_feed_animals)

    def choice_feed_animals(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.FEED_ANIMALS.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_FEED_ANIMALS:
