...
from keyboards.city.for_animal_home import get_keyboard_animal_home
...
class AnimalHome(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_animal_home(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_CITY,
                              reply_markup=get_keyboard_animal_home())
        self.bot.register_next_step_handler(msg, self.choice_animal_home)

    def choice_animal_home(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.ANIMAL_HOME.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_ANIMAL_HOME:
