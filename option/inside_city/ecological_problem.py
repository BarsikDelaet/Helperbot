...
from keyboards.city.for_ecological_problem import get_keyboard_ecological_problem
...
class EcologicalProblem(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_ecological_problem(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_CITY,
                              reply_markup=get_keyboard_ecological_problem())
        self.bot.register_next_step_handler(msg, self.choice_ecological_problem)

    def choice_ecological_problem(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.ECOLOGICAL_PROBLEM.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_ECOLOGICAL_PROBLEM:
