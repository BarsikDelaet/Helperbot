...
from keyboards.city.for_people_street import get_keyboard_people_street
...
class PeopleStreet(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_people_street(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_CITY,
                              reply_markup=get_keyboard_people_street())
        self.bot.register_next_step_handler(msg, self.choice_people_street)

    def choice_people_street(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.PEOPLE_STREET.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_PEOPLE_STREET:
