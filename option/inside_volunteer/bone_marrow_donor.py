...
from keyboards.volunteer.for_bone_marrow_donor import get_keyboard_bone_marrow_donor
...
class BoneMarrowDonor(Service):

    """Создание и обработка всех кнопок раздела."""
    def menu_bone_marrow_donor(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_VOLUNTEER,
                              reply_markup=get_keyboard_bone_marrow_donor())
        self.bot.register_next_step_handler(msg, self.choice_bone_marrow_donor)

    def choice_bone_marrow_donor(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.VOLUNTEER_MARROW_DONOR.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_MARROW_DONOR:
