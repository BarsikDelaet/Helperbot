"""Кнопки и информация о донора крови."""
...
from keyboards.volunteer.for_blood_donor import get_keyboard_blood_donor
...
class BloodDonor(Service):

    """Создание и обработка всех кнопок раздела."""
    def menu_blood_donor(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_VOLUNTEER,
                              reply_markup=get_keyboard_blood_donor())
        self.bot.register_next_step_handler(msg, self.choice_blood_donor)

    def choice_blood_donor(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.VOLUNTEER_BLOOD_DONOR.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_BLOOD_DONOR:
