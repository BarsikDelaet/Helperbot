"""Кнопки и информация о Волонтерство офлайн."""
...
from keyboards.volunteer.for_volunteer_offline import get_keyboard_volunteer_offline
...
class VolunteerOffline(Service):

    """Создание и обработка всех кнопок раздела."""
    def menu_volunteer_offline(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_VOLUNTEER,
                              reply_markup=get_keyboard_volunteer_offline())
        self.bot.register_next_step_handler(msg, self.choice_volunteer_offline)

    def choice_volunteer_offline(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.VOLUNTEER_OFFLINE.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_OFFLINE:
