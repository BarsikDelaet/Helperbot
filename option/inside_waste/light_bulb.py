"""Кнопки и информация о лампочка."""

from option.service import Service
from telebot import types
import random
from option.inside_waste import answer
import answer_main
from keyboards import general
from keyboards.waste import keyboard
from keyboards.waste.for_light_bulb import get_keyboard_light_bulb
...
class LightBulb(Service):
    ...
    def menu_light_bulb(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_WASTE,
                              reply_markup=get_keyboard_light_bulb())
        self.bot.register_next_step_handler(msg, self.choice_light_bulb)

    def choice_light_bulb(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.CHOICE_WASTE_LIGHT_BULB.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_LIGHT_BULB:
    ...
    def choice_repeat_or_next(self, msg):
        ...
        if msg.text.lower() == general.CHOICE_REPEAT[0]:
            self.bot.send_message(msg.chat.id, answer.CHOICE_REPEAT_OR_NEXT_WASTE,
            ...
            self.bot.send_message(msg.chat.id,answer.WASTE_MSG,
