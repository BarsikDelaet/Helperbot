"""Кнопки и информация о пластике."""
...
from keyboards.waste.for_plastic import get_keyboard_plastic
...
class Plastic(Service):

    """Создание и обработка всех кнопок раздела."""
    def menu_plastic(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_WASTE,
                              reply_markup=get_keyboard_plastic())
        self.bot.register_next_step_handler(msg, self.choice_plastic)

    def choice_plastic(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.CHOICE_WASTE_PLASTIC.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_PLASTIC:
