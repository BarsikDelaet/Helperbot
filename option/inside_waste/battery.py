"""Кнопки и информация о батарейках."""
...
from keyboards.waste.for_battery import get_keyboard_battery
...
class Battery(Service):
    ...
    def menu_battery(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_WASTE,
                              reply_markup=get_keyboard_battery())
        self.bot.register_next_step_handler(msg, self.choice_battery)

    def choice_battery(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.CHOICE_WASTE_BATTERY.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_BATTERY:
