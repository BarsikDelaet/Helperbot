...
from keyboards.waste.for_glass import get_keyboard_glass
...
class Glass(Service):
    ...
    def menu_glass(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_WASTE,
                              reply_markup=get_keyboard_glass())
        self.bot.register_next_step_handler(msg, self.choice_glass)

    def choice_glass(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.CHOICE_WASTE_GLASS.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_GLASS:
