...
from keyboards.helper.for_adults import get_keyboard_adults
...
class Adults(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_adults(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_HELPER,
                              reply_markup=get_keyboard_adults())
        self.bot.register_next_step_handler(msg, self.choice_adults)

    def choice_adults(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.ADULTS.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_ADULTS:
