...
from keyboards.helper.for_animal import get_keyboard_animal
...
class Animal(Service):
    ...
    def menu_animal(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_HELPER,
                              reply_markup=get_keyboard_animal())
        self.bot.register_next_step_handler(msg, self.choice_animal)

    def choice_animal(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.ANIMAL.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_ANIMAL:
