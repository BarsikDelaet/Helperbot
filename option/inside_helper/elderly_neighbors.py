...
from keyboards.helper.for_elderly_neighbors import get_keyboard_elderly_neighbors
...
class ElderlyNeighbors(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_elderly_neighbors(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_HELPER,
                              reply_markup=get_keyboard_elderly_neighbors())
        self.bot.register_next_step_handler(msg, self.choice_elderly_neighbors)

    def choice_elderly_neighbors(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.ELDERLY_NEIGHBORS.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_ELDERLY_NEIGHBORS:
