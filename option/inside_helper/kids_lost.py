...
from keyboards.helper.for_kids_lost import get_keyboard_kids_lost
...
class KidsLost(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_kids_lost(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_HELPER,
                              reply_markup=get_keyboard_kids_lost())
        self.bot.register_next_step_handler(msg, self.choice_kids_lost)

    def choice_kids_lost(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.KIDS_LOST.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_KIDS_LOST:
