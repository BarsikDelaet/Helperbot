...
from keyboards.helper.for_woman import get_keyboard_woman
...
class Woman(Service):
    ...
    def menu_woman(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_HELPER,
                              reply_markup=get_keyboard_woman())
        self.bot.register_next_step_handler(msg, self.choice_woman)

    def choice_woman(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.WOMAN.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_WOMAN:
