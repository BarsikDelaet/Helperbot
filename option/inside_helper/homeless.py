...
from keyboards.helper.for_homeless import get_keyboard_homeless
...
class Homeless(Service):
    """Создание и обработка всех кнопок раздела."""

    def menu_homeless(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_HELPER,
                              reply_markup=get_keyboard_homeless())
        self.bot.register_next_step_handler(msg, self.choice_homeless)

    def choice_homeless(self, msg):
        """Обработка кнопок."""
        text_msg = msg.text.lower()
        chat_id = msg.chat.id

        result_msg = answer.HOMELESS.get(text_msg, answer_main.UNDERSTAND_MSG)

        if text_msg in keyboard.CHOICE_HOMELESS:
