"""Кнопки и информация о <<отдаче>> детской одежды."""
...
from keyboards.give_away.for_clothing_kids import get_keyboard_clothing_kids
...
class ClothingKids(Service):

    """Создание и обработка всех кнопок раздела."""
    def menu_clothing_kids(self, msg):
        """Меню с кнопками."""
        self.bot.send_message(msg.chat.id, answer.HELLO_INSIDE_GIVE_AWAY,
                              reply_markup=get_keyboard_clothing_kids())
        self.bot.register_next_step_handler(msg, self.choice_clothing_kids)

    def choice_clothing_kids(self, msg):
        """Обработка кнопок."""
    ...
    def address_find(self, msg):
        ...
        msg_id = self.bot.send_message(msg.chat.id, "Ищу...",
                              reply_markup=types.ReplyKeyboardRemove())
        result = find_in_city(msg.text, answer.ITEM_CLOTHING_KIDS)
...
Фото/Текс/Документ.""",
    ...
    def processing_repeat_or_next(self, msg):
        ...
        text_msg = msg.text.lower()
        if text_msg == text_msg == general.CHOICE_REPEAT_OR_NEXT[0]:
            citizens = 0
